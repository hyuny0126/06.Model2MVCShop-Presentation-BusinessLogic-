	...
	<c:set var="i" value="0" />
		<c:forEach var = "purchase" items="${list}">
			...
			<tr class="ct_list_pop">
				...
				<!-- 현재상태 항목(배송코드에 따라)  -->	
				<c:choose>
					<c:when test="${purchase.tranCode=='0'}">구매가 최소되었습니다.</c:when>
					<c:when test="${purchase.tranCode=='1'}">현재 구매완료 상태입니다.
						<a href="/updateTranCode.do?tranNo=${purchase.tranNo}&tranCode=0">구매취소 </a>
					</c:when>
					<c:when test="${purchase.tranCode=='2'}">현재 배송중입니다.</c:when>
					<c:when test="${purchase.tranCode=='3'}">현재 배송완료 상태입니다.
						<a href="/updateTranCode.do?tranNo=${purchase.tranNo}&tranCode=-1">반품신청 </a>
					</c:when>
					<c:when test="${purchase.tranCode=='-1'}">반품신청이 완료되었습니다. 기다려주세요.</c:when>
					<c:when test="${purchase.tranCode=='-3'}">반품신청이 불가합니다.죄송합니다.</c:when>
